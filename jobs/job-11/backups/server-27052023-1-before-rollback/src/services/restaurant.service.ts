...
export default class RestaurantService {
   ...
   * to be ready to start to work. Also, it runs a background schedular that check if there are any
   * waiting orders to handle while all workers at this stage were busy.
   * @return {void}
   */
  public static initiate() {
    ...
    // Initiate the waiting list interval schedular.
    setInterval(async () => {
      // Process the order by stages.
      const theOrders = await this.getAllWaitingOrders();
      ...
      theOrders.forEach(async (theOrder: any) => {
        this.processStage(
          theOrder,
          theOrder.stage === STAGE.PENDING ? STAGE.DOUGH : theOrder.stage
        );
      });
      // ToDo: Insert this into the config.
    }, 1000);
  }
  ...
  private static async processStage(order: any, stage: STAGE): Promise<void> {
    try {
      ...
      if (stage !== STAGE.PENDING && stage !== STAGE.ERROR) {
        workerIndex = this.getWorkerIndex(WORKER_TYPE as WORKER);
        // Validate that there is available worker to process the stage.
        if (workerIndex === null || workerIndex === -1 || order.status !== STATUS.WAITING) {
          // Update the status of the stage to PENDING and process it later.
  ...
  private static async getAllWaitingOrders(): Promise<any[]> {
    return OrderModel.find({ status: STATUS.WAITING }).sort({ createdAt: 1 });
  }
